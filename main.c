...
void game_loop_iter() {
    // Generate a random last line
    for (int x = 0; x < WINDOW_WIDTH; x++) {        
        screen[WINDOW_HEIGHT-1][x] = abs(32768 + rand()) % 256;
    }
    
    // Calculate the pallete index for each pixel
    for (int y = 0; y < WINDOW_HEIGHT - 1; y++) {
        for (int x = 0; x < WINDOW_WIDTH; x++) {            
            screen[y][x] =
                ((screen[(y + 1) % WINDOW_HEIGHT][(x - 1 + WINDOW_WIDTH) % WINDOW_WIDTH]
                + screen[(y + 1) % WINDOW_HEIGHT][(x) % WINDOW_WIDTH]
                + screen[(y + 1) % WINDOW_HEIGHT][(x + 1) % WINDOW_WIDTH]
                + screen[(y + 2) % WINDOW_HEIGHT][(x) % WINDOW_WIDTH])
                * FIRE_A) / FIRE_B;
            buffer[y][x] = RGBToInt(pallete[screen[y][x]]);
        }        
    }
    
    // Dump memory into the texture and show it
...
int main(int argc, char ** argv) {
  ...
  window = SDL_CreateWindow("fire_wasm_sdl",
